...
import { FiPhone, FiArrowUpRight, FiMenu, FiX } from "react-icons/fi";
import HeaderSearch from "../components/HeaderComponents/HeaderSearch.jsx";
import newLogoMittler from "../media/newLogoMittler.png";
...
function Header() {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            // Sichtbarkeits-Logik:
            // 1. Wenn gescrollt wird, Header verstecken (außer wir sind ganz oben)
            if (window.scrollY > 100) {
                setIsVisible(false);
            }

            // 2. Timer zurücksetzen: Wenn 250ms nicht mehr gescrollt wurde, Header zeigen
    ...
    // Verhindert das Scrollen des Hintergrunds bei offenem Menü
    useEffect(() => {
        if (isMenuOpen) {
        ...
        >
            ...
            <div className="relative mx-auto w-full max-w-[1700px] px-5 py-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:max-w-[1900px] 2xl:px-20 min-[2200px]:max-w-[2200px] min-[2200px]:px-24 min-[2200px]:py-6">
                ...
                <div className="flex items-center justify-between gap-8">
                    ...
                    >
                        <img src={newLogoMittler} alt="Logo" className="h-10 w-auto md:hidden" />
                        <img src={LogoMittlerTabletQuery} alt="Logo" className="hidden h-14 w-auto md:block lg:hidden" />
                        <img src={LogoMittlerLaptopQuery} alt="Logo" className="hidden h-14 w-auto lg:block xl:h-16 2xl:h-20 min-[2200px]:h-24" />
                    </NavLink>
                    ...
                    <div className="hidden flex-1 items-center justify-between md:flex">
                        ...
                        <div className="relative z-10 flex items-center gap-6 lg:gap-8 xl:gap-10">
                            ...
                            >
                                ...
                                <span className="hidden lg:inline">01575 3137765</span>
                            </a>
                            ...
                            <div className="flex items-center gap-3 xl:gap-4">
                                ...
                                >
                                    Jetzt anfragen <FiArrowUpRight className="transition-transform group-hover:-translate-y-0.5 group-hover:translate-x-0.5" />
                                </Link>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 md:hidden">
                        <a href="tel:015753137765" className="text-[#2AA34D]"><FiPhone size={22} /></a>
                        <button 
                            onClick={() => setIsMenuOpen(!isMenuOpen)}
                            className="text-[#083224] relative z-[60]"
                        >
                ...
                <nav className="relative z-10 mt-6 hidden border-t border-[#F0F4F1] pt-6 md:block">
                    <ul className="flex items-center justify-between lg:justify-start lg:gap-12 xl:gap-16">
                        {[
                            ...
                            { name: "Montage & Hausmeister", path: "/montage-hausmeisterarbeiten" },
                ...
                {/* Mobile Menu Overlay */}
                <div className={`fixed inset-0 top-[72px] z-50 transform bg-white transition-transform duration-500 md:hidden h-[calc(100vh-72px)] overflow-y-auto ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}>
                    <nav className="flex min-h-full flex-col p-6">
                        <ul className="space-y-2">
                            ...
                            <li><NavLink to="/montage-hausmeisterarbeiten" className={mobileNavLinkClass} onClick={closeMenu}>Montage & Hausmeister</NavLink></li>
                        ...
                        <div className="mt-auto pt-10 pb-12 space-y-4">
                            <Link to="/kontakt" onClick={closeMenu} className="flex h-14 items-center justify-center bg-[#083224] font-bold uppercase tracking-widest text-white">Jetzt anfragen</Link>
                            <Link to="/karriere" onClick={closeMenu} className="flex h-14 items-center justify-center border border-[#D9E7DC] font-bold uppercase tracking-widest text-[#083224]">Karriere</Link>
                            <a href="tel:015753137765" className="flex h-14 items-center justify-center border border-[#D9E7DC] font-bold text-[#083224]">01575 3137765</a>
                        </div>
                    </nav>
                </div>
            </div>
